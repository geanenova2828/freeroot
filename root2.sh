...
case $install_ubuntu in
  [yY][eE][sS])
    # Substituição do wget por curl + tar stream
    curl --retry $max_retries --retry-delay $timeout -sL \
...
if [ ! -e $ROOTFS_DIR/.installed ]; then
  ...
  # Substituição do wget por curl com retry manual
  for i in $(seq 1 $max_retries); do
    curl -sSL --retry 3 --retry-delay $timeout \
